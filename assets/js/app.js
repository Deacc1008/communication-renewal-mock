...
function scheduleRows(){
  ...
  DB.campaigns.forEach(c=>{
    const start=c.startDate||DB.TODAY, end=c.endDate||add(DB.TODAY,45);
    const campaignExpired=end<DB.TODAY || c.status==="Expired";
    c.isExpired=campaignExpired;
    if(campaignExpired || c.status==="Paused") return;
    campaignTriggers(c).forEach(t=>{
      DB.subs.forEach(s=>{
        ...
        if(execution>=start && execution<=end && execution>=DB.TODAY){
...
function Templates(){
  const type=DB.templateType||"WhatsApp";
  const list=DB.templates.filter(x=>x.type===type);
  let t=DB.templates.find(x=>x.id===DB.template && x.type===type)||list[0];
  if(t) DB.template=t.id;
  ...
  const preview=type==="WhatsApp"
    ? `<div class="phone"><div class="phonehead">KDK Support · WhatsApp</div><div class="bubble">${text.replace(/{{1}}/g,"Aarav").replace(/{{2}}/g,"Spectrum Cloud Professional").replace(/{{3}}/g,"23 Sep 2026").replace(/{{4}}/g,"7")}</div></div>`
    : `<div class="email-preview"><div class="mail-head"><b>${esc(t.subject||"Renewal reminder")}</b><span>To: customer@example.com</span></div><div class="mail-body">${text.replace(/{{1}}/g,"Aarav").replace(/{{2}}/g,"PDF Signer Business").replace(/{{3}}/g,"25 Sep 2026").replace(/{{4}}/g,"7")}</div></div>`;
  ...
  <section class="card editor"><div class="title"><h3>${esc(t.name)}</h3><span class="muted">${esc(t.product)} · <span class="status-dot"></span> Approved</span></div>
  <div class="field"><label>Template Name</label><input value="${esc(t.name)}"></div>
  ${type==="Email"?`<div class="field"><label>Email Subject</label><input value="${esc(t.subject||"")}"></div>`:""}
  <div class="field"><label>Content</label><textarea style="width:100%;min-height:125px">${esc(t.text||"")}</textarea></div>
  <div class="callout"><b>Provider:</b> ${type==="WhatsApp"?"Rampwin API → Meta approved template":"ZeptoMail → Email template"}<br><span class="muted">Provider callbacks are simulated.</span></div></section>
...
function Scheduler(){
  ...
  const all=DB.campaigns.map(c=>{c.isExpired=(c.endDate||add(DB.TODAY,45))<DB.TODAY;return c;});
  ...
  <section class="card panel"><div class="summary-grid"><div class="summary-box">Simulation Date<b>${fmt(DB.TODAY)}</b></div><div class="summary-box">Active Schedules<b>${all.filter(c=>c.status==="Scheduled"||c.status==="Running").length}</b></div><div class="summary-box">Upcoming Executions<b>${rows.length}</b></div></div>
  ${scheduleFull()}</section>`;
}
function scheduleFull(){
  let r=scheduleRows();
  const campaignRows=DB.campaigns.map(c=>({c,expired:(c.endDate||add(DB.TODAY,45))<DB.TODAY||c.status==="Expired"}));
  return `<div style="overflow:auto;margin-top:14px"><table class="table"><tr><th>Execution Date</th><th>Trigger</th><th>Campaign</th><th>Eligible</th><th>WhatsApp</th><th>Email</th><th>State</th><th>Next Action</th></tr>`+
  ...
  (r.length?"":`<tr><td colspan="8" class="muted" style="padding:35px;text-align:center">No upcoming execution rows for the selected date range. Check the campaign start/end dates and triggers.</td></tr>`))+
  `</table></div>
  <div class="card" style="margin-top:14px;padding:12px"><b>Campaign schedule status</b><div style="overflow:auto"><table class="table"><tr><th>Campaign</th><th>Start</th><th>End</th><th>Status</th><th>Action</th></tr>${campaignRows.map(x=>`<tr><td>${esc(x.c.name)}</td><td>${fmt(x.c.startDate)}</td><td>${fmt(x.c.endDate)}</td><td><span class="badge ${x.expired?"gray":"g"}">${x.expired?"Expired":x.c.status}</span></td><td>${x.expired?`<button class="btn small" onclick="scheduleDetails(${x.c.id})">View Scheduled Details</button>`:`<button class="btn small" onclick="stopSchedule(${x.c.id})">Stop Schedule</button>`}</td></tr>`).join("")}</table></div></div>`;
}
function History(){
...
function campaignModal(){
  ...
  <div class="field"><label>Organisation Type</label><select id="co" onchange="setCampaignOrg(this.value)"><option ${c.organisationType==="Both"?"selected":""}>Both</option><option ${c.organisationType==="DIY"?"selected":""}>DIY</option><option ${c.organisationType==="DIFM"?"selected":""}>DIFM</option></select><div class="help">Choose DIY, DIFM or Both.</div></div></div>
  <div class="input-row"><div class="field"><label>Campaign Start Date</label><input id="csd" type="date" value="${esc(c.startDate||DB.TODAY)}"><div class="help">No execution occurs before this date. Example: next Monday.</div></div>
  <div class="field"><label>Send Time</label><input id="ct" type="time" value="${esc(c.time||"10:00")}"><div class="help">The selected time is used for each trigger execution.</div></div></div>
  <div class="field"><label>Campaign End Date</label><input id="ced" type="date" value="${esc(c.endDate||add(DB.TODAY,45))}"><div class="help">After this date the campaign is treated as expired and has no next action.</div></div>
...
function saveCampaign(){
  ...
  c.id=Date.now();c.status=c.endDate<DB.TODAY?"Expired":"Scheduled";
...
function stopSchedule(id){let c=DB.campaigns.find(x=>x.id===id);if(!c)return;c.status="Paused";toast("Schedule stopped.");}
function scheduleDetails(id){let c=DB.campaigns.find(x=>x.id===id);if(!c)return;DB.modal={schedule:c};render();}
function scheduleModal(){
 const c=DB.modal.schedule;
 return `<div class="modalbg"><div class="modal"><div class="head"><div><h2>Scheduled Details</h2><p>Campaign configuration and calculated execution rules.</p></div><button class="btn" onclick="DB.modal=null;render()">×</button></div>
 ...
 <div class="field"><label>Product</label><input value="${c.products.length===4?"All Products":c.products.join(", ")}" readonly></div>
 <div class="field"><label>Trigger Points</label><div>${campaignTriggers(c).map(t=>`<span class="trigger-chip">${triggerLabel(t)}</span>`).join("")}</div></div>
 <div class="field"><label>Channels / Send Time</label><input value="${esc(c.channels.join(" + "))} · ${esc(c.time)} IST" readonly></div>
 <div class="callout">Eligibility is recalculated from Subscription Master at execution time. Renewed customers are excluded from later triggers.</div>
 <div class="actions"><button class="btn primary" onclick="DB.modal=null;render()">Close</button></div></div></div>`;
}
function historyDetails(i){
 const a=[{d:"16 Sep 2026",c:"September Renewal Journey",ch:"WhatsApp",n:84,s:"Delivered"},{d:"16 Sep 2026",c:"September Renewal Journey",ch:"Email",n:72,s:"Opened"},{d:"09 Sep 2026",c:"August Renewal Campaign",ch:"WhatsApp",n:126,s:"Delivered"},...DB.msg];
 DB.modal={history:a[i]};render();
}
function historyModal(){
 const h=DB.modal.history;
 return `<div class="modalbg"><div class="modal"><div class="head"><div><h2>Communication Details</h2><p>Simulated audit/provider details.</p></div><button class="btn" onclick="DB.modal=null;render()">×</button></div>
 <div class="summary-grid"><div class="summary-box">Date<b>${esc(h.d)}</b></div><div class="summary-box">Channel<b>${esc(h.ch)}</b></div><div class="summary-box">Status<b>${esc(h.s)}</b></div></div>
 <div class="field"><label>Campaign</label><input value="${esc(h.c)}" readonly></div>
 <div class="field"><label>Communication Count</label><input value="${esc(h.n)}" readonly></div>
 <div class="callout"><b>Provider response:</b> ${h.ch==="WhatsApp"?"Rampwin API / Meta template callback simulated.":"ZeptoMail delivery/open event simulated."}<br><span class="muted">This prototype does not make live provider API calls.</span></div>
 <div class="actions"><button class="btn primary" onclick="DB.modal=null;render()">Close</button></div></div></div>`;
}
